...
function setup() {
  ...
  eye = loadImage("eye(1-2).png");
  ear = loadImage("ear(3-4).png");
  textSize(16);
}
    ...
    for (let j = 0; j < pose.keypoints.length; j++) {
      ...
      if (keypoint.score > 0.2) {
        ...
        if (j == 1 || j == 2) {
          push();
          imageMode(CENTER);
          image(eye,keypoint.position.x, keypoint.position.y, 50, 50);
          pop();
          //text("NOSE", keypoint.position.x + -15, keypoint.position.y + 15);
        }
        if (j == 3 || j == 4) {
          push();
          imageMode(CENTER);
          image(ear,keypoint.position.x, keypoint.position.y, 50, 50);
          pop();
          //text("NOSE", keypoint.position.x + -15, keypoint.position.y + 15);
        }
      }
